 ...
 <div class="card">
     ...
     <!-- /.card-header -->
     <div class="card-body">
         <form action="<?= isset($edit) ? route_to('laporan.update', $pendaftaran['id_laporan']) : route_to('laporan.save') ?>" method="post">
             <?= csrf_field() ?>
             ...
             <div class="form-group">
                 ...
                 <textarea class="form-control <?= ($validation->hasError('hasil_diagnosa')) ? 'is-invalid' : null ?>" value="<?= old('hasil_diagnosa') ?: (isset($edit) ? $laporan['hasil_diagnosa'] : null) ?>" rows="2" name="hasil_diagnosa" placeholder="Tulis Diagnosa" autofocus></textarea>
                 <span id="exampleInputEmail1-error" class="error invalid-feedback"><?= $validation->getError('hasil_diagnosa') ?></span>
             </div>
     </div>
